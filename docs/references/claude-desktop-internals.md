...
**Known account UUIDs (this machine):**

| UUID prefix | Account | Email | Session type | Sessions |
|-------------|---------|-------|--------------|----------|
| `36fbf886` | roamhq | mack@roamhq.io | Local | 10 |
| `c70a361f` | evolution7 | mack@evolution7.com.au | Local | 22 |
| `f5554caa` | mknv74 | mknv74@gmail.com | **Remote (cloud VM)** | 40 |
...
```json
{
  "sessionId":        "local_2ec49b58-dbc8-4363-9d3a-3c73bdcdbfb6",
  "cliSessionId":     "932ce07a-b55a-4c29-9054-438e765e131f",
  "cwd":              "X:\\Forge\\claude-mods",
  "originCwd":        "X:\\Forge\\claude-mods",
...
```json
{
  "worktrees": {
    "<worktree-name>": {
      "name":         "strange-sinoussi-184b95",
      "path":         "X:\\Forge\\Axiom\\.claude\\worktrees\\strange-sinoussi-184b95",
      "leasedBy":     "local_7315fee4-a31e-4067-bd62-160e91ef3eb0",
      "baseRepo":     "X:\\Forge\\Axiom",
      "branch":       "claude/strange-sinoussi-184b95",
      "sourceBranch": "main",
...
**Observed accounts on this machine:**
- evolution7 (`c70a361f`) — runs **local** sessions (cwd: `X:\Forge\...`)
- mknv74 (`f5554caa`) — runs **remote** sessions (cwd: `/sessions/<vm-name>/mnt`)
...
Transferred sessions (copied from another account's dir) appear via path #2 — the filesystem scan. Confirmed: 7 of 8 transferred Axiom sessions were ABSENT from `react-query-cache-ls` but appeared in the sidebar. The 8th appeared in the cache only as a previously-cached transcript (`epitaxy.local.transcript.*` key), not as a session list entry.
...
**Tested transfers (this machine, 2026-04-26):**
- evolution7 (`c70a361f`) → roamhq (`36fbf886`): hackathon coach (29 turns, 1.8MB JSONL) — appeared, opened, accepted new messages ✓
- evolution7 (`c70a361f`) → roamhq (`36fbf886`): 8× X:\Forge\Axiom sessions (0–132 turns) — bulk copied ✓
...
for ws in "$EVO"/*/; do
  for f in "$ws"*.json; do
    [ -f "$f" ] || continue
    cwd=$(jq -r '.cwd // ""' "$f")
    [ "$cwd" = "X:\\Forge\\TargetProject" ] || continue
