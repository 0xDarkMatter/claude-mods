...
**23 agents. 78 skills. 13 styles. 4 hooks. 6 rules. One install.**
...
**v2.7.0** (May 2026)
- 🩺 **`mac-ops` skill** - Comprehensive macOS workstation operations, peer to `windows-ops` with the same conventions retuned for macOS's failure modes. 12 scripts: `health-audit` (the orchestrator), `panic-triage` (decodes `.panic`/`.ips` reports with kext provenance + pre-panic timeline), `startup-audit` (Login Items + LaunchAgents + LaunchDaemons + privileged helpers + profiles inventory), `safe-disable-startup` (cross-mechanism, no sudo for user-scope, reversible via `launchctl disable/enable`), `disk-health` (APFS verify + IO error log scan + SMART), `drive-dependencies` ("safe to eject?" — open files / TM target / media libraries / launchd plist refs), `boot-perf` (boot duration via unified log + slow-launchd events), `recover-clone` (rsync `--partial --inplace --no-whole-file --append-verify --ignore-errors` — the macOS analogue of windows-ops's `robocopy /R:0`), plus four mac-unique scripts: `tcc-audit` (reads both TCC.dbs, classifies grants/denials, surfaces the most common "this app doesn't work" cause), `wake-reasons` (pmset log breakdown by class — rtc/Maintenance vs wlan vs BT.HID vs user input), `spotlight-status` (mds/mdworker health per volume), `storage-pressure` (APFS snapshots + local Time Machine + purgeable space breakdown — explains the "disk full but du says otherwise" mystery). Seven reference docs catalog the long tail: `storage-events.md` (unified-log IO error patterns), `panic-codes.md` (panic strings, kext provenance, Apple Silicon vs Intel), `startup-mechanisms.md` (vendor patterns for Adobe/Docker/Google/Zoom), `recovery-patterns.md` (rsync vs ditto vs ddrescue decision tree), `tcc-mechanics.md` (full kTCCService catalog, `tccutil` reset semantics, Full Disk Access requirement), `launchd-deep-dive.md` (`disable` vs `bootout` vs `unload`), `remote-diagnostics.md` (SSH-only — no WinRM analog). 8-rung diagnostic ladder adds TCC as rung 7 (the macOS-unique privacy permissions dimension). 68-test self-suite. Dogfooded on this Mac during build: surfaced a real ULA-only IPv6 + router DoH blocking pattern, plus quirks in pmset log format and plutil error-handling that got patched in real time.
